...
public interface TrabajadorRepo extends JpaRepository<TrabajadorEntity, Long> {
	...
	@Query("""
			...
			""")
	Page<TrabajadorEntity> findByFiltros(@Param("texto") String texto, @Param("estado") String estado,
			@Param("idCategoria") Long idCategoria, Pageable pageable);
	...
	@Query("""
			...
			""")
	Page<TareaEntity> findTareasByTrabajador(@Param("trabajadorId") Long trabajadorId, Pageable pageable);
}
